...
import { CouponCreatedListener } from './events/listeners/coupon/coupon-created-listener';
import { CouponUpdatedListener } from './events/listeners/coupon/coupon-updated-listener';
import { CouponDeletedListener } from './events/listeners/coupon/coupon-deleted-listener';
...
import { OrderCartDeletedListener } from './events/listeners/order/order-cart-deleted-listener';
...
(async () => {
  ...
  try {
    ...
    new CouponUpdatedListener(natsWrapper.client).listen();
    new CouponDeletedListener(natsWrapper.client).listen();

    new ProductCreatedListener(natsWrapper.client).listen();
    new ProductUpdatedListener(natsWrapper.client).listen();
    new ProductDeletedListener(natsWrapper.client).listen();

    new OrderCartDeletedListener(natsWrapper.client).listen();
    ...
    logger.error(`Error is : ${e} From Cart Service`);
  }

  const PORT = 3000 || process.env.PORT;
  app.listen(PORT, () =>
    logger.info(`Server Listening On Port ${PORT} From Cart Service`)
  );
})();
